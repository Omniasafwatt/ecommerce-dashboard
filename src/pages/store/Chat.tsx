...
import { Send, Package, Image as ImageIcon } from 'lucide-react'
...
export default function StoreChat() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Order Chats</h1>
        ...
        <div className="flex h-full">
          ...
          <div className="w-72 border-r border-gray-200 flex flex-col flex-shrink-0">
            <div className="overflow-y-auto flex-1">
              {storeThreads.map(t => (
                <button key={t.id} onClick={() => handleSelect(t)} className={`w-full text-left p-3 border-b border-gray-50 hover:bg-gray-50 ${selected?.id === t.id ? 'bg-blue-50' : ''}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <p className="text-sm font-medium text-gray-900 truncate">{t.customer}</p>
                        {t.unread > 0 && <span className="w-4 h-4 bg-blue-600 text-white text-xs rounded-full flex items-center justify-center">{t.unread}</span>}
                      </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {selected.messages.map(msg => (
                  <div key={msg.id} className={`flex flex-col ${msg.sender === 'store' ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-xs px-3 py-2 rounded-xl text-sm ${msg.sender === 'store' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900'}`}>
                      {msg.text}
                    </div>
