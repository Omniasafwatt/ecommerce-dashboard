  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('nav.bundles', 'Bundles')}</h1>
          <p className="text-sm text-gray-500 mt-1">{items.length} {t('common.bundlesConfigured', 'bundles configured')}</p>
        </div>
        <Link to="/admin/catalog/bundles/new" className="flex items-center gap-2 px-4 py-2 bg-sky-500 text-white rounded-lg text-sm font-medium hover:bg-sky-600">
          <Plus size={16} />{t('common.addBundle', 'Add Bundle')}
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="p-4 border-b border-gray-100">
          <div className="relative max-w-xs">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search bundles..." className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500" />
          </div>
        </div>
            ...
            <div key={item.id} className="p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  ...
                  <Link to={`/admin/catalog/bundles/${item.id}`} className="p-1.5 text-gray-400 hover:text-sky-600 hover:bg-blue-50 rounded-lg"><Pencil size={14} /></Link>
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 className="font-semibold text-gray-900 mb-2">{t('common.deleteBundle', 'Delete Bundle?')}</h3>
            <p className="text-sm text-gray-500 mb-5">{t('common.cannotUndo', 'This action cannot be undone.')}</p>
